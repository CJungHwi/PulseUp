...
-- 공지사항 삭제
DELIMITER //
CREATE OR REPLACE PROCEDURE sp_DeleteAnnouncement(
    IN p_id BIGINT
)
BEGIN
    START TRANSACTION;
...
-- 공지사항 상태 변경
DELIMITER //
CREATE or replace PROCEDURE sp_UpdateAnnouncementStatus(
    IN p_id BIGINT,
    IN p_status VARCHAR(20)
)
BEGIN
...
-- 사용자별 메뉴 생성 프로시저 (승인 시 역할 기준 기본 메뉴 등록)
DELIMITER //
...
BEGIN
    ...
    WHERE m.is_active = TRUE 
      ...
      AND (
        m.target_audience IN ('all', 'user')
        OR (v_user_role IN ('branch_admin', 'super_admin') AND m.target_audience = 'branch_admin')
        OR (v_user_role = 'super_admin' AND m.target_audience = 'super_admin')
      )
    ON DUPLICATE KEY UPDATE 
      is_enabled = TRUE,
      updated_at = NOW();
...
DELIMITER //
create or replace PROCEDURE sp_GetPopularWorkouts(
