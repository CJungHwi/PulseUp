...
CREATE TABLE menus (
    ...
    target_audience ENUM('all', 'admin', 'user', 'branch_admin') NOT NULL DEFAULT 'all' COMMENT '대상 사용자 그룹 - all: 전체, admin: 관리자, user: 일반사용자, branch_admin: 지점관리자',
...
CREATE TABLE users (
    ...
    role ENUM('user', 'admin', 'super_admin') NOT NULL DEFAULT 'user' COMMENT '사용자 역할 - user: 일반사용자, admin: 관리자, super_admin: 최고관리자',
