/**
 * AMRAP: 인트로 프리뷰 정렬. 영상 그리드는 intro-grid-sequences의 첫 메인 라운드 정렬.
 */
...
export const collectAmrapIntroSequences = <T extends { round: number | string; position?: string }>(
  allExercises: T[],
  mainRound: number,
): T[] => collectFirstMainSortedIntroGridSequences(allExercises, mainRound)

export const buildAmrapPreviewFirstRoundOrdered = (
  _exercises: any[],
  _minRound: number,
...
export const primeAmrapIntroStartPreload = (
