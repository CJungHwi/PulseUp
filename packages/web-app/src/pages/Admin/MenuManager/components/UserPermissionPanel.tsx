/**
 * UserPermissionPanel — 선택 사용자의 정보 + 메뉴 권한 패널
 *
 * 기능:
 * - 좌측: 사용자 기본 정보 (아이디/이름/이메일/역할/지점)
 * - 우측: 사용자 메뉴 권한 리스트 (메뉴 sort_order 오름차순 정렬)
 *
 * 정렬:
 * - menus.sort_order ASC, 동률 시 menu_name ASC (안정 정렬)
 *
...
export const UserPermissionPanel: React.FC<UserPermissionPanelProps> = ({
    ...
    onTogglePermission,
    selectedMenuNodeId,
    userTypeLabel: userTypeLabelProp,
    isAdminPanel = false
}) => {
    const userTypeLabel = userTypeLabelProp ?? (isAdminPanel ? '관리자' : '사용자')

    const sortedPageMenuItems = useMemo(() => {
        return userMenuItems
            .filter((item) => item.menu_path)
            .slice()
            .sort((a, b) => {
        ...
        <div className="flex gap-[3px] h-full">
            ...
            <Card className="flex-1 flex flex-col min-w-0 shadow-md border-[#343637] dark:border-[#6b7280] bg-card overflow-hidden">
                ...
                <CardContent className="p-0 flex-1 overflow-hidden">
                    {sortedPageMenuItems.length > 0 ? (
                        <div className="h-full overflow-y-auto p-2 space-y-1">
                            {sortedPageMenuItems.map((item) => (
                                <div
                                    key={item.id}
                                    className={cn(
