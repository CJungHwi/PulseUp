 ...
 *
 * 흐름: 월 범위 계산 → 수업/내 예약 병렬 조회 → 슬롯 선택 → 정원/중복 상태 확인 → 예약 저장.
 */
...
export const ClassBookingCalendar: React.FC = () => {
  ...
  const reservedSlotIds = useMemo(() => {
    return new Set(myBookings.filter((booking) => booking.status === 'reserved').map((booking) => booking.slot_id))
  }, [myBookings])
  ...
  const canReserve = (slot: ClassSlot) => (
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      showSnackbar({ message: '수업예약이 완료되었습니다.', severity: 'success' })
  ...
  return (
    <div className="relative h-[calc(100vh-140px)] p-0 flex flex-col gap-[3px] overflow-hidden bg-background">
      <BookingMonthCalendar
        ...
        getSlotBadge={(slot) => {
          if (reservedSlotIds.has(slot.id)) return { label: '예약완료', color: 'primary' }
          if (isPastStart(slot.start_at)) return { label: '종료', color: 'default' }
          if (getRemainingCapacity(slot) <= 0) return { label: '마감', color: 'default' }
