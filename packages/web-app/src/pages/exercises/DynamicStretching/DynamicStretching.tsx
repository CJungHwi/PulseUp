 ...
 *
 * 흐름: 날짜·마스터 선택 → 상세 편집 → 저장 API → 필요 시 삭제.
 */
...
export const DynamicStretching: React.FC = () => {
  ...
  const [exerciseType, setExerciseType] = useState('DS')
  ...
  const convertServerPositionToDS = (serverPosition: string): string => {
    const mapping: { [key: string]: string } = { 'L1': 'DS1', 'L2': 'DS2', 'L3': 'DS3', 'R1': 'DS4', 'R2': 'DS5', 'R3': 'DS6' }
    return mapping[serverPosition] || serverPosition
  }

  const convertDSPositionToServer = (dsPosition: string): string => {
    const mapping: { [key: string]: string } = { 'DS1': 'L1', 'DS2': 'L2', 'DS3': 'L3', 'DS4': 'R1', 'DS5': 'R2', 'DS6': 'R3' }
    return mapping[dsPosition] || dsPosition
  }

  const getPositionByIndex = (index: number): string => `DS${index + 1}`
  ...
  const handleApply = async (master?: WorkoutMaster) => {
    ...
    setExercises(details.map((detail, index) => ({
      ...
      purpose: detail.purpose && detail.purpose.trim() !== '' ? detail.purpose : 'Dynamic Stretching',
