/**
 * 페이지 요약 — 다이내믹 스트레칭 (`/DynamicStretching`)
 *
 * 기능: 동적 스트레칭 운동 편집·Vimeo 재생·운동 이력 조회·저장·삭제.
 *
 * 호출/연동:
 ...
 *
 * 관련 컴포넌트(`./components/`):
 * - `DynamicStretchingMasterPanel`: 좌측(검색 + 마스터/상세 + 수직 스플리터)
 * - `DynamicStretchingEditorPanel`: 우측 상단(편집 테이블 + 액션)
 * - `DynamicStretchingPreviewPanel`: 우측 하단(영상 미리보기 + 메모)
 * - `useDynamicStretchingVimeoPlayer`: Vimeo 플레이어 훅
 * - `useSplitLayout`: 좌우/상하 스플리터 훅
 * - `dynamicStretchingPositionUtils.ts`, `dynamicStretchingTypes.ts`
 *
...
import {
  ...
  WorkoutDetail,
  WorkoutMaster,
} from './components/dynamicStretchingTypes'
import {
  convertDSPositionToServer,
  convertServerPositionToDS,
  getPositionByIndex,
  reassignPositions,
} from './components/dynamicStretchingPositionUtils'
import { useDynamicStretchingVimeoPlayer } from './components/useDynamicStretchingVimeoPlayer'
import { useSplitLayout } from './components/useSplitLayout'
import { DynamicStretchingMasterPanel } from './components/DynamicStretchingMasterPanel'
import { DynamicStretchingEditorPanel } from './components/DynamicStretchingEditorPanel'
import { DynamicStretchingPreviewPanel } from './components/DynamicStretchingPreviewPanel'
...
export const DynamicStretching: React.FC = () => {
  ...
  const { iframeRef, isPlayerReady, isPlaying, handlePlayPause, handleReplay } =
    useDynamicStretchingVimeoPlayer({
      ...
      details.map(
        (detail, index) =>
          ({
            ...
            purpose: detail.purpose || 'Dynamic Stretching',
    ...
    try {
      ...
      const saveData = {
        ...
        exercises: exercises.map((ex, index) => ({
          ...
          position: convertDSPositionToServer(ex.position || getPositionByIndex(index)),
          exercise_type: 'DS',
        })),
        workoutExercises: [],
