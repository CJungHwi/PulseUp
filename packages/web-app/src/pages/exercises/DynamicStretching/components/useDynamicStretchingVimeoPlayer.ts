/**
 * useDynamicStretchingVimeoPlayer — Vimeo 플레이어 초기화 + 구간 반복 훅
 *
 * 입력:
 ...
 *
 * 사용처: `DynamicStretching.tsx` → `DynamicStretchingPreviewPanel.tsx`
 */
import Player from '@vimeo/player'
import { useEffect, useRef, useState } from 'react'

interface UseDynamicStretchingVimeoPlayerArgs {
  videoUrl?: string | null
  videoStartTime?: number
  videoEndTime?: number
}

export const useDynamicStretchingVimeoPlayer = ({
  videoUrl,
  videoStartTime = 0,
  videoEndTime = 0,
}: UseDynamicStretchingVimeoPlayerArgs) => {
